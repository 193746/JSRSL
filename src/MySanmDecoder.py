...
class ParaformerSANMDecoder(BaseTransformerDecoder):
    ...
    def __init__(
        ...
        self.span_tasks=span_task.split("+")
        
        # ner
        # if "ner" in self.span_tasks:
        self.span_dense = nn.Linear(inner_dim, (self.inner_dim * 2))
        self.linear_start = nn.Linear(self.inner_dim,1)
        self.linear_end = nn.Linear(self.inner_dim,self.name_entity_num+1)
        # ic
        if "ic" in self.span_tasks:       
            self.classify_dense = nn.Linear(self.inner_dim,self.inner_dim*2)
            self.proj_action = nn.Linear(self.inner_dim,1)
            self.proj_scenario=nn.Linear(self.inner_dim,1)
            self.action_out = nn.Linear(self.inner_dim,self.action_num)
            self.scenario_out = nn.Linear(self.inner_dim,self.scenario_num)  
    ...
    def forward(
        ...
        # use refiner
        embedding_layer=self.refiner.get_input_embeddings()
        inputs_embeds=torch.matmul(x,embedding_layer.weight)
        attention_mask=tgt_mask.squeeze(-1).int()
        token_type_ids=torch.zeros(attention_mask.shape,dtype=torch.int32).to(x.device)
        hidden_state=self.refiner(inputs_embeds=inputs_embeds,attention_mask=attention_mask,token_type_ids=token_type_ids).last_hidden_state
        batch_size=len(hidden_state[:,0,0])
        seq_len=len(hidden_state[0,:,0])

        # ner outputs:(batch_size, seq_len, inner_dim*2)
        outputs = self.span_dense(hidden_state)
        qw,kw=self.make_rope(batch_size,seq_len,outputs)
        start_logits = self.linear_start(qw)
        start_logits = torch.squeeze(start_logits, -1)
        # [b,l]
        start_prob = self.sigmoid(start_logits)
        # [b,l,4]
        end_prob = self.linear_end(kw)

        if "ic" not in self.span_tasks:
            return x, olens, start_prob, end_prob, None, None

        # classifier
        outputs=self.classify_dense(hidden_state)
        qw,kw=self.make_rope(batch_size,seq_len,outputs)
        # attn_weights:(batch_size, seq_len)
        # out:(batch_size, inner_dim)
        attn_action = self.proj_action(qw).squeeze(-1).softmax(dim=-1)
        action_out = torch.einsum("bs,bsi->bi", attn_action.float(), qw.float())
        # target_prob:(batch_size,type_num)
        action_prob=self.action_out(action_out)
        attn_scenario = self.proj_scenario(kw).squeeze(-1).softmax(dim=-1)
        scenario_out = torch.einsum("bs,bsi->bi", attn_scenario.float(), kw.float())
        # target_prob:(batch_size,type_num)
        scenario_prob=self.scenario_out(scenario_out)
 
        return x, olens, start_prob, end_prob, action_prob, scenario_prob
