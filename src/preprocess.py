...
class CommonPreprocessor(AbsPreprocessor):
    ...
    def _text_process(
        ...
        if self.text_name in data and self.tokenizer is not None:
            ...
            tokens_map = get_tokens_map(tokens)
            text_ints = self.token_id_converter.tokens2ids(tokens)
            data[self.text_name] = np.array(text_ints, dtype=np.int64)
        # word level span -> token level span
        if "start" in data and "end" in data:
            tokens_start,tokens_end=span_word2token(data["start"],data["end"],tokens_map)
            data["start"]=np.array(tokens_start,dtype=np.float32)
            data["end"]=np.array(tokens_end,dtype=np.float32)
        return data
