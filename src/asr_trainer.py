...
class ASRTrainer(BaseTrainer):
    ...
    def evaluate(self,*args,**kwargs):
        output_path = os.path.join("output",datetime.datetime.now().strftime("%m-%d-%H-%M-%S")+"|"+self.dataset_name+"|"+self.model_dir.split("/")[-1])
        os.makedirs(output_path,exist_ok=True)
        logger.info(f"Inference result will be saved in {output_path}.")
        wav_dict={}
        wav_path=os.path.join("dataset",self.dataset_name,"train_data/test/wav.scp")
        with open(wav_path,"r",encoding="UTF-8") as f:
            line=f.readline()
            while line:
                index,wav=line[:-1].split(" ")
                if self.dataset_name=="AISHELL-NER":
                    index=str(int(re.sub(r"[a-zA-Z]", "", index)))
                else:
                    index=index[1:]
                wav=wav.split("/")[-1]
                wav_dict[index]=wav
                line=f.readline()
        self.trainer.evaluate(output_path,wav_dict)
